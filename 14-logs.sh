...
then 
   ...
   echo "You are running wit root access" &>>$LOG_FILE
fi
...
then 
   ...
   VALIDATE $? "MYSQL"
    ...
    echo -e "Nothing to do MySQL... $Y is already isntalled $N" &>>$LOG_FILE
fi
...
then 
   ...
   echo -e "Nothing to do python... $Y is already installed $N" &>>$LOG_FILE
fi
